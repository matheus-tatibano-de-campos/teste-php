...
Desenvolvido em **PHP nativo**, seguindo o padrão **MVC** e Orientação a Objetos, sem uso de frameworks ou Composer.
...
Inicie o serviço do MySQL (pelo painel do XAMPP ou via terminal). No phpMyAdmin ou cliente MySQL de sua preferência, importe o arquivo:

```
database/schema.sql
```

Isso cria o banco `jm_servicos`, as tabelas e o usuário inicial.
...
- Se colocar em `C:\xampp\htdocs\teste-php\teste-php`: mantenha o padrão `http://localhost-php/teste-php/`
...
**Via Apache (XAMPP):**
Copie a pasta para o seu `htdocs` e acesse no navegador a URL configurada no `BASE_URL` (ex: `http://localhost/teste-php/teste-php/` ou `http://localhost/teste-php/`).
...
> Lembre-se de colocar `define('BASE_URL', 'http://localhost:8080/');` no `config/config.php`.
...
1. Baixe e execute o **Mailpit** (`mailpit.exe`).
2. No arquivo `php.ini` do seu PHP/XAMPP, configure a seção de e-mail para apontar para a porta local do Mailpit:
...
3. Reinicie o Apache.
4. Ao finalizar qualquer serviço no dashboard, abra no navegador:

```
http://localhost:8025
```

para ver a mensagem recebida.
...
- Proteção de rotas com validação de sessão ativa
